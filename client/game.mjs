...
export function startGame(levelMap, gameState) {
  ...
  ship = new Ship(
    [gameState.ship.y, gameState.ship.x],
    [home.y, home.x],
    gameState.ports.filter((port) => !port.isHome),
    gameState.prices,
    gameState.goodsInPort,
    levelMap,
  );
}
...
class Ship {
  ...
  initActions() {
    ...
    this.stopGreedy =
      this.minRoadToPort * 5 + 6 > this.maxTurn
        ? 0
        : this.minRoadToPort * 5 + 6;
  ...
  createTwoBestExpeditions() {
    ...
    this.ports.forEach((firstPort) => {
      this.ports.forEach((secondPort) => {
        ...
        variant.ports = [
          JSON.parse(JSON.stringify(firstPort)),
          JSON.parse(JSON.stringify(secondPort)),
        ];
        ...
        if (
          variants.some((variantForSearch) => {
        ...
        Object.defineProperty(variant, 'stepsLeft', {
          get: function() {
            return (
              ship.maxTurn -
              (this.ports[0].road.length * 2 +
                this.ports[1].road.length +
                this.ports[0].itemsForTravel.length * 2 +
                this.ports[1].itemsForTravel.length * 2 +
                ship.turn)
            );
          },
        });
            ...
            if (variant.items[nameOfGood]) {
              ...
              Object.defineProperties(itemData, {
                ...
                maxAmountOfItems: {
                  get: function() {
                    return Math.min(
                      Math.floor(this.leftSpaces / this.volume),
                      variant.items[nameOfGood].amount,
                    );
                  },
                },
                moneyProfit: {
        ...
        while (variant.stepsLeft > 1) {
          ...
          variant.prices = variant.prices.filter((singlePrice) => {
            return singlePrice.maxAmountOfItems > 0;
          });
          ...
          const maxAmount = item.maxAmountOfItems;
          variant.total += item.moneyProfit;
          variant.ports[item.portIndex].leftSpaces -= maxAmount * item.volume;
          variant.items[item.name].amount -= maxAmount;
        }
  ...
  constructExpeditionsForEveryPort() {
    ...
    this.prices.forEach((portPrices) => {
      ...
      for (const goodNumber in goodsAvailable) {
        ...
        goodsAvailable[goodNumber].profit =
          portPrices[goodName] / goodsAvailable[goodNumber].volume;
      }
      ...
      for (let i = 0; i < itemsForBuy.length; i += 1) {
        ...
        let amountOfItems = Math.floor(
          leftSpace / goodsAvailable[itemNumber].volume,
        );
        if (amountOfItems === 0) {
          continue;
        }
        ...
        const possibleProfit =
          (expedition.price +
            amountOfItems * goodsAvailable[itemNumber].price) /
          (expedition.distance + 2);
        if (expedition.profit > possibleProfit) {
          continue;
        }
        expedition.items.push({
          name: goodsAvailable[itemNumber].name,
  ...
  generatePathsMap() {
    this.pathMap = new Array(this.map.length)
      .fill(null)
      .map(() => new Array(this.map[0].length).fill(null));
    ...
    while (stack.length) {
      ...
      this.arrayOfMoves.forEach((move) => {
        ...
        if (
          this.map[newCoord[0]][newCoord[1]] === '#' ||
  ...
  createExtraInfoForPorts() {
    this.ports.forEach((port) => {
      ...
      // Переворачиваем путь
      port.road = [...port.backRoad]
        .reverse()
        .map((direction) => this.reversedDirection[direction]);
    });
    this.ports = this.ports.filter((port) => port.road.length);
  }
  ...
  changeActionFromPirates(actionToDo) {
    const nextPosition = [
      this.currentPos[0] + this.reversedMapMoves[actionToDo][0],
    ...
    if (!this.positionSafeFromPirates(nextPosition)) {
      if (!this.positionSafeFromPirates(this.currentPos)) {
        let possibleMoves = this.arrayOfMoves
          .map((i) => [i[0] + this.currentPos[0], i[1] + this.currentPos[1]])
        ...
        const commandMove = this.mapMoves[
          [
  ...
  createPath(fromCooridantes) {
    ...
    while (currentPosition.prev) {
      const move = [
        currentPosition.prev[0] - positionCoordinates[0],
      ...
      currentPosition = this.pathMap[currentPosition.prev[0]][
        currentPosition.prev[1]
      ];
    }
  ...
  findBestExpedition() {
    ...
    while (
      expedition.road.length + expedition.items.length * 2 >
    ...
    if (this.logger) {
      ...
      console.log(
        ...
        this.turn,
        '-',
        expForLogs.distance + this.turn,
        `(${expForLogs.distance})`,
      );
      console.log('PortId: ', expForLogs.portId);
      console.log(
        'Items: ',
        expForLogs.items.map((i) => `${i.name}: ${i.amount}`).join(', '),
      );
  ...
  isValidPos(pos) {
    return (
      pos[0] >= 0 &&
      pos[0] < this.map.length &&
      pos[1] >= 0 &&
      pos[1] < this.map[0].length
    );
  }
